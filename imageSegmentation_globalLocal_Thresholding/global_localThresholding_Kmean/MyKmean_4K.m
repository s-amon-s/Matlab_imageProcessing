...
text = sprintf('No. of times taken = %d',i);
  disp(text);
  
  maxx = [max(clusterA) max(clusterB) max(clusterC) max(clusterD)];
  minn = [min(clusterA) min(clusterB) min(clusterC) min(clusterD)];
...
function [arr brr crr drr] = adjustCluster(a,meanA,meanB,meanC,meanD)
 ...
 for i=1:(size(a,1)*size(a,2))
          [mn cluster] = min([abs(double(a(i))- double( meanA)) abs(double(a(i))- double(meanB)) abs(double(a(i))- double(meanC)) abs(double(a(i))- double(meanD))]);
          if cluster == 1
                arr(end+1) = a(i);
          elseif cluster == 2
               brr(end+1) = a(i);
