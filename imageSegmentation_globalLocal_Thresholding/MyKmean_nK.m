...
%% Initialize workspace, Start timer
   clc;
   tic; % start timer
%% Read image
...
while true
    ...
    add = cell(1,k);
    for i=1:k
        adjusted = adjustCluster(clusters{i},means); %group pixels in to right cluster, based on K-MEAN
        clusters{i} = adjusted{i};
        for j=1:k
            if j~=i
                add{j} = cat(2,add{j},adjusted{j});                
            end
            if i==k
...
function section = adjustCluster(cluster,means)
    ...
    for i=1:(size(cluster,1)*size(cluster,2))
        ...
        [mn loc] = min(chk);
        section{loc} = cat(2,section{loc},cluster(i));
    end
end
